...
class SearchResultLinkBuilderTest {
  ...
  void buildSearchResultLinks_Should_returnSearchResultLinks_When_allParametersAreSet() {
    ...
    assertThat(
        searchResultLinks.getPrevious().getHref(),
    ...
    assertThat(
        searchResultLinks.getSearch().getHref(), is("/agencyadmin/agencies?page=2&perPage=20{&q}"));
  }
  ...
  void buildSearchResultLinks_Should_returnPlaceHolderSelfLink_When_keywordIsNotSet() {
    var searchResultLinks =
        SearchResultLinkBuilder.getInstance().withPage(1).withPerPage(20).buildSearchResultLinks();

    assertThat(searchResultLinks, notNullValue());
    assertThat(
        searchResultLinks.getSelf().getHref(), is("/agencyadmin/agencies?page=1&perPage=20{&q}"));
  }

  @Test
  void buildSearchResultLinks_Should_returnDefaultPaginationValues_When_noParametersAreSet() {
    var searchResultLinks = SearchResultLinkBuilder.getInstance().buildSearchResultLinks();
