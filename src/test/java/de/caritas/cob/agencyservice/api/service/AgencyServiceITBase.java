...
public class AgencyServiceITBase {
  ...
  public void getAgencies_Should_returnMatchingAgencies_When_postcodeAndConsultingTypeIsGiven()
    ...
    when(consultingTypeManager.getConsultingTypeSettings(CONSULTING_TYPE_PREGNANCY))
        .thenReturn(CONSULTING_TYPE_SETTINGS_PREGNANCY);
    String postCode = "88662";

    List<FullAgencyResponseDTO> resultAgencies =
        agencyService.getAgencies(
            Optional.of(postCode),
            CONSULTING_TYPE_PREGNANCY,
            Optional.empty(),
            Optional.empty(),
            Optional.empty(),
            Optional.empty());
  ...
  public void setAgencyOffline_Should_FlagAgencyAsOfflineAndSetUpdateDate() {
    ...
    Agency agencyBefore = agencyRepository.findById(883L).orElseThrow(RuntimeException::new);
    agencyService.setAgencyOffline(883L);
    Agency agencyAfter = agencyRepository.findById(883L).orElseThrow(RuntimeException::new);
