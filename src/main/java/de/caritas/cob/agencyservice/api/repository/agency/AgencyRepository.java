...
public interface AgencyRepository extends JpaRepository<Agency, Long> {
  ...
  String SELECT_WITH_TOPICS =
      "SELECT a.*, :tenantId FROM agency a "
          ...
          + "AND CAST(SUBSTR(r.postcode_to, 1, :length) AS int)))) "
          + "AND a.is_offline = false "
  ...
  String SELECT_ALL_AGENCIES_TOPICS =
      "SELECT distinct(at.topic_id) FROM agency_topic at "
          + "INNER JOIN agency a ON a.id = at.agency_id ";

  String GROUP_BY_ORDER_BY = "GROUP BY a.id " + "ORDER BY a.postcode DESC";
  ...
  @Query(value = SELECT_ALL_AGENCIES_TOPICS + ORDER_BY_TOPIC, nativeQuery = true)
